...
def save_files_final(directory: str, arrays: List[Tuple[str, torch.Tensor]], index: int):
    """Save a bunch of tensors."""
    for (arr_name, arr) in arrays:
...
def compute_sample_losses(network_O: nn.Module, network: nn.Module, loss_functions: List[nn.Module], dataset: Dataset,
                   batch_size: int = DEFAULT_PHYS_BS):
    """Compute loss over a dataset."""
    L = len(loss_functions)
    print("This is the sample loss being calculated, there is the the length of the dataset", L)
    losses = [0. for l in range(L)]
    lossesO = [0. for l in range(L)]
    with torch.no_grad():
        for (X, y) in iterate_dataset(dataset, batch_size):
            preds = network(X)
            for l, loss_fn in enumerate(loss_functions):
    ...
    with torch.no_grad():
        for (X, y) in iterate_dataset(dataset, batch_size):
            preds = network_O(X)
            for l, loss_fn in enumerate(loss_functions):
                lossesO[l] += loss_fn(preds, y) / len(dataset)
    losses_dif = [lo -l for lo, l in zip(lossesO, losses)]
    return losses_dif



def compute_max_loss(network_O: nn.Module, network: nn.Module, loss_functions: List[nn.Module], dataset: Dataset,
                     batch_size: int = DEFAULT_PHYS_BS):
    """Compute maximum loss over a dataset for all loss functions."""
    max_loss = -float('inf')
    with torch.no_grad():
        for (X, y) in iterate_dataset(dataset, batch_size):
            preds = network(X)
            ...
            for loss_fn in loss_functions:
                ...
                lossO = loss_fn(predsO, y).item()
                loss_diff = abs(lossO-loss)
                if loss_diff > max_loss:
                    max_loss = loss
    return max_loss
...
def compute_gradient(network: nn.Module, loss_fn: nn.Module,
    ...
    p = len(parameters_to_vector(network.parameters()))
    average_gradient = torch.zeros(p, device='cpu')
    for (X, y) in iterate_dataset(dataset, physical_batch_size):
